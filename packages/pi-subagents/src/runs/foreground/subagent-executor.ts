...
} from "./subagent-request-normalizer.ts";
export type { SubagentParamsLike, TaskParam } from "./subagent-request-types.ts";
export { applyAgentDefaultContext, expandTopLevelTaskCounts, normalizeRepeatedParallelCounts } from "./subagent-request-normalizer.ts";
...
async function runForegroundParallelTasks(input: ForegroundParallelRunInput): Promise<SingleResult[]> {
	return mapConcurrent(input.tasks, input.concurrencyLimit, async (task, index) => {
		...
		const taskCwd = resolveParallelTaskCwd(task, input.paramsCwd);
		const readPrefix = behavior && Array.isArray(behavior.reads) && behavior.reads.length > 0
			? `[Read from: ${behavior.reads.map((file) => path.isAbsolute(file) ? file : path.join(taskCwd, file)).join(", ")}]\n\n`
			: "";
		...
		const taskText = injectSingleOutputInstruction(
			`${readPrefix}${input.taskTexts[index]!}`,
			outputPath,
		);
...
async function runParallelPath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details>> {
	...
	const foregroundControl = deps.state.foregroundControls.get(runId);
	{
		...
		const parallelProgressPrecreated = false;
		...
		const downgradeNote = backgroundRequestedWhileClarifying ? " (background requested, but kept foreground)" : "";
...
export function createSubagentExecutor(deps: ExecutorDeps): {
	...
	const execute = async (
		...
		const surfaceRequest = surface.request;
		...
		if (surfaceRequest.kind === "interrupt") {
			const targetRunId = surfaceRequest.targetRunId;
			...
			if (foreground?.interrupt) {
				...
				if (interrupted) {
					...
					return { content: [{ type: "text", text: `Interrupt requested for foreground run ${foreground.runId}.` }], details: { mode: "management", results: [] } };
				}
				return { content: [{ type: "text", text: `Foreground run ${foreground.runId} has no active child step to interrupt.` }], isError: true, details: { mode: "management", results: [] } };
			}
			const asyncInterruptResult = interruptAsyncRun(deps.state, targetRunId);
			if (asyncInterruptResult) return asyncInterruptResult;
			return { content: [{ type: "text", text: "No interrupt-capable run found in this session." }], isError: true, details: { mode: "management", results: [] } };
		}

		const { blocked, depth, maxDepth } = checkSubagentDepth(deps.config.maxSubagentDepth);
		if (blocked) {
			return {
				content: [{ type: "text", text: `Nested subagent call blocked (depth=${depth}, max=${maxDepth}). You are running at the maximum subagent nesting depth. Complete your current task directly without delegating to further subagents.` }],
				isError: true,
				details: { mode: "single" as const, results: [] },
			};
		}
		...
		const scope: AgentScope = "both";
		const discoveredAgents = deps.discoverAgents(shape.request.effectiveCwd, scope).agents;
		const contextShape = applyDefaultContextToRunShape(shape.request, discoveredAgents);
		let effectiveParams = contextShape.params;
		...
		const agents = intercomBridge.active
			? discoveredAgents.map((agent) => applyIntercomBridgeToAgent(agent, intercomBridge))
			: discoveredAgents;
		const normalizedRun = validateSubagentRunRequest({ shape: contextShape, executionAgents: agents });
		...
		const hasSingle = runRequest.mode === "single";
