...
import { isParallelStep, resolveStepBehavior, suppressProgressForReadOnlyTask, type ChainStep, type ResolvedStepBehavior, type SequentialStep, type StepOverrides } from "../../shared/settings.ts";
...
export function executeAsyncChain(
	...
	const buildSeqStep = (s: SequentialStep, sessionFile?: string, resolvedBehavior?: ResolvedStepBehavior) => {
		...
		const readPrefix = Array.isArray(behavior.reads) && behavior.reads.length > 0
			? `[Read from: ${behavior.reads.map((file) => path.isAbsolute(file) ? file : path.join(stepCwd, file)).join(", ")}]\n\n`
			: "";
		const outputPath = resolveSingleOutputPath(behavior.output, ctx.cwd, stepCwd);
		const validationError = validateFileOnlyOutputMode(behavior.outputMode, outputPath, `Async step (${s.agent})`);
		if (validationError) throw new AsyncStartValidationError(validationError);
		const task = injectSingleOutputInstruction(`${readPrefix}${s.task ?? "{previous}"}`, outputPath);
	...
	try {
		steps = chain.map((s, stepIndex) => {
			if (isParallelStep(s)) {
				...
				return {
					parallel: s.parallel.map((t, taskIndex) => buildSeqStep(t, nextSessionFile(), parallelBehaviors[taskIndex])),
